...
if length > 3:
    print ("Invalid argument. The syntax is pygen.py <stringLength> <numberOfPasswords>")
    sys.exit(0)

# error handling, defaults to a string length of 12 
try:
    n = int(sys.argv[1]) # string length
    x = int(sys.argv[2]) # number of passwords to generate
except IndexError:
    n = 14 # default to a string length of 12
